...
# ╔═╡ 1fbee930-8622-11ef-24f3-03df4307dfa3
begin
	using CairoMakie
	using LinearAlgebra
	using PlutoUI
...
# ╔═╡ cabadc1c-524d-4f9d-bcc1-ad1c58a026fe
# Basic nyoom
begin
	function nyoom(h, q)
		heights = []
...
# ╔═╡ c968207d-ff53-4283-8449-2ceb605fe7fc
# nyoom with variable velocity
begin
...
# ╔═╡ c0c9b71a-720d-4474-893f-686bd50d109e
...
begin
	...
	heights = [10.0, 20.0, 30.0]
	velocities = [2.0, 3.0, 4.0]
	query_count = 5
	
	nyoom(heights, velocities, query_count)
end

# ╔═╡ e2edecbd-d2a3-4a01-83bc-66e43b38c9d9
# Bind the variable q to a slider
@bind q PlutoUI.Slider(1:10)

# ╔═╡ ebb869c2-6e5a-48e7-a679-fa9b081b0484
nyoom(heights, velocities, q)

# ╔═╡ 45cc016b-0109-41ef-8bdf-ffe3bd995d9a
md"""q = $(q)"""

# ╔═╡ 6e324814-1e2b-422d-83b9-e8450a829f77
# Black Bird's Visual Request
begin
	f = Figure()
	ax = Axis(f[1, 1], 	 
	...
	xlabel = "Time (s)",
	ylabel = "Height",)
	scatter!(ax, 1:q, nyoom(1, q))
	# save("simple_shot.png", f)
	f
end

# ╔═╡ c3700a29-78cb-419d-b0c2-0dd5be5ab4fd
# Plotting the output of the third function
begin
	h = rand(10)
	v = rand(10)	
	f_third = Figure()
	...
	for result in results
		scatter!(ax_third, 1:q, result)
	end
	# save("scatter_shot.png", f_third)
	f_third
end

# ╔═╡ 6b052d1a-5902-4e8e-9b1f-8cac3fc18fae
...
# ╔═╡ 620cf3f6-107f-4f50-97e0-87097fff15c6
# Spline Vector k
...
# ╔═╡ f1e41e74-cfcb-4ce8-8b1c-3c7622c951ae
...
# ╔═╡ 496b81a4-33b5-476a-91fd-b412ece8bb99
# White Bird's Request
begin
	...
	x_knots = Vector(1:8)
	y_knots = Vector([trajectory(i) for i in x_knots])
	splines = gen_spline(x_knots, y_knots)

	# Original f(x)
	x_vals = range(1, 8, length=100)
	y_vals = [trajectory(x) for x in x_vals]
	fig_wb = Figure()
	ax_wb = Axis(fig_wb[1,1], title = "White Bird's Ambitious Request", xlabel = "x", ylabel = "f(x)")
	...
	# save("spline.png", fig_wb)
	fig_wb
end

# ╔═╡ 6b82e5e6-436a-43d0-bb89-ba723b89d062
# k slider
@bind k PlutoUI.Slider(8:20, show_value = true)

# ╔═╡ a98465ea-aebf-434f-9fff-3bac0b4ea80f
# ip slider
@bind ip PlutoUI.Slider(1:24, show_value = true)

# ╔═╡ 2a2a1b54-e7b6-4d9b-afbd-37ee33de9e32
# White Bird's Request with Parametrization
begin
	x_knots_param = Vector(1:k)
